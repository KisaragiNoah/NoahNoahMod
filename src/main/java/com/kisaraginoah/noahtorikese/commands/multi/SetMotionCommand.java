...
@EventBusSubscriber
public class SetMotionCommand {
    @SubscribeEvent
    public static void registerCommand(RegisterCommandsEvent event) {
        CommandDispatcher<CommandSourceStack> dispatcher = event.getDispatcher();
        dispatcher.register(Commands.literal("setmotion")
    ...
    private static int setmotion(CommandSourceStack sourceStack, Collection<? extends Entity> targets, float motionx, float motiony, float motionz) {
        int successCount = 0;

        for (Entity entity : targets) {
            if (entity instanceof LivingEntity livingEntity) {
                livingEntity.setDeltaMovement(new Vec3(motionx, motiony, motionz));
                successCount++;
            }
        }
